...
from backend.core import AllModelEnum, OpenAIModelName
...
class ChatMessage(BaseModel):
    ...
    additional_kwargs: dict[str, Any] = Field(
        description="Transport for backend-specific message metadata (e.g. visibility flags).",
...
class DBSearchInput(BaseModel):
    """Schema for DB search input parameters."""

    query: str = Field(
        ...,
        description="SQL query on the DB in string",
    )
    table_name: str = Field(
        ...,
        description="Name of the DB table to query",
    )
